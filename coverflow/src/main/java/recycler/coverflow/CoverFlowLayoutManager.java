...
public class CoverFlowLayoutManager extends RecyclerView.LayoutManager {
    ...
    /**모든 항목의 오프셋 정보를 위, 아래, 왼쪽 및 오른쪽으로 저장*/
    private SparseArray<Rect> mAllItemFrames = new SparseArray<>();

    /**아이템이 화면에 나타나고 재활용되지 않았는지 기록하세요.
     * true는 화면에 표시되어 재활용되지 않았음을 의미합니다.*/
    private SparseBooleanArray mHasAttachedItems = new SparseBooleanArray();

    /**RecyclerView의 Item 리사이클러*/
...
//            cm.setSaturation(0.9f);
    ...
    private void startScroll(int from, int to) {
        ...
        mAnimation.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationStart(Animator animation) {
                // 여기에서 바꾸는 소스코드를 진행하면 될듯 싶다.
                mSelectedListener.onAnimationStart(getCenterPosition());
                Log.d("YJP","onAnimationStart");
                Log.d("YJP","getLastVisiblePosition : " + String.valueOf( getLastVisiblePosition()));

            }

            @Override
            public void onAnimationEnd(Animator animation) {
                Log.d("YJP","onAnimationEnd");
                onSelectedCallBack();
            }

            @Override
            public void onAnimationCancel(Animator animation) {
                Log.d("YJP","onAnimationCancel");

            }

            @Override
            public void onAnimationRepeat(Animator animation) {
                Log.d("YJP","onAnimationRepeat");
    ...
    /**
     * 마지막으로 보이는 아이템의 위치를 얻는다
     * <p>Note:항목은 표시 영역에서 마지막으로 그려진 항목이며, 두 번째 항목으로 인해 차단 될 수 있습니다
     */
